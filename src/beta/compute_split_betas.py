"""
📈 calc_beta_split.py – ניתוח ביטא אסימטרית ומדדי קשר לשוק

🔍 מטרה:
קובץ זה מחשב מדדים חשובים לקשר בין כל מניה לבין השוק (S&P 500),
תוך הבחנה בין עליות וירידות בשוק. המטרה היא לזהות מניות עם קשר משמעותי
(או ניתוק מובהק) להתנהגות השוק.

🧾 קלט (INPUT):
- טבלת daily_stock_data עם נתוני מניות, כולל עמודת 'split' עם ערך 'train'
- טבלת sp500_index עם נתוני מדד השוק, כולל עמודת 'split' עם ערך 'train'

📤 פלט (OUTPUT):
- קובץ CSV בשם beta_split_with_corr_r2.csv עם העמודות הבאות:
    - symbol: סימול המניה
    - beta_pos: ביטא בימים שהשוק עולה
    - beta_neg: ביטא בימים שהשוק יורד
    - beta_diff: ההפרש בין beta_pos ל־beta_neg
    - correlation: מתאם בין תשואת המניה לשוק
    - r_squared: אחוז השונות של תשואת המניה המוסברת ע"י השוק

📊 תרשים:
- מציג את הקשר בין correlation ל־R² עבור כל מניה
- קווים אדומים/כתומים מציינים ספים לסינון מניות מנותקות מהשוק

🛠️ מיועד לבחינת התאמה לשוק כחלק ממערך אלגוטריידינג.
"""
